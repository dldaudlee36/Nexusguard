# 🛡️ GIGANG 엔드투엔드 전체 운영 시나리오 가이드 (Notion 전용)
...
GIGANG는 이 문제를 **"사전 선제 감시(WATCH) ➔ 실제 유출 시 AI 즉시 대응(HIGH) ➔ 오탐 시 시스템 자가 치유(NORMAL)"**라는 3단계 동적 라이프사이클로 해결합니다.
...
| :--- | :--- | :--- |
| **마케팅팀 김대리** | `user_marketing`<br>(IP: `192.168.10.45`) | 업무를 빨리 끝내려고 사내 고객 DB를 조회한 뒤 외부 ChatGPT에 통째로 올려 분석하려는 내부 직원 |
...
| **GIGANG 시스템** | 중앙 수집 & 상관분석 엔진<br>(Railway + SQLite + Gemini) | 김대리의 단말기, DB 감사 로그, 방화벽 트래픽을 실시간 교차 검증하는 보안 관제 플랫폼 |
...
* **시스템 동작**:
  * 단말의 `GIGANGAgent.exe`가 백그라운드에서 주기적으로 DNS 질의와 웹 접근 이벤트를 수집하여 Railway 중앙 서버로 전송합니다.
...
* **실시간 시스템 반응**:
  ...
  * 영속 DB(`risk_history`)에 감사 이력 자동 기록:  
    `[14:05:22] user_marketing | NORMAL ➔ WATCH | 사내 기밀 DB 조회 선행 후 미승인 AI 접속`
...
아직 돈을 건네주지 않았더라도, 경비원(GIGANG)은 이 사람을 당장 <b>'요주의 감시 대상(WATCH)'</b>으로 등록하고 주시해야 합니다. 기존 보안 장비는 돈을 다 건네주고 난 뒤에야 알람을 울렸지만, GIGANG는 문을 여는 순간 이미 감시를 시작합니다.
</aside>
...
* **시스템의 동작**:
  ...
  * 영속 DB 기록:  
    `[14:35:00] user_marketing | WATCH ➔ NORMAL | 30분 무전송 만료(TTL)로 인한 정상 자가 치유`
* **도입 효과**:
  * 기존 보안 관제는 오탐 알람이 하루 수천 건씩 쌓여 관제사를 지치게 만들었지만(Alert Fatigue), GIGANG는 시스템이 알아서 무해한 상황을 판정하고 스스로 청소합니다.
...
보안팀이 모든 AI 사이트를 막아버리면 직원들은 스마트폰 테더링이나 개인 노트북으로 몰래 사용합니다(풍선 효과). GIGANG의 거버넌스 탭은 이를 **"양성화"**합니다.
...
GIGANG는 단순 목업(Mockup)이 아닌, 실제 동작하는 파이프라인입니다.

```
[팀원 윈도우 PC] 
   └── GIGANGAgent.exe (DNS 및 웹 접속 감지)
          ↓ (암호화 HTTPS POST 통신)
[클라우드 중앙 수집 서버] (Railway Cloud / Flask + PostgreSQL)
          ↓ (실시간 JSON 폴링 & 스트리밍)
[GIGANG 코어 엔진] (correlation.py & sqlite_store.py)
   ├── 1. Unified JSON 정규화
...
**Q1. 기존 EDR이나 DLP 솔루션과 비교했을 때 GIGANG만의 결정적인 차별점은 무엇인가요?**
> **답변**: 기존 DLP는 사내 업무망 전체의 AI 접속을 차단하여 업무 생산성을 심각하게 떨어뜨리고, SIEM은 유출이 끝난 수 시간 뒤에야 로그를 취합해 알려줍니다. GIGANG는 **'기밀 DB 조회'와 '미승인 SaaS 접속'을 시간 축으로 실시간 교차 검증**하여, 평소에는 직원의 AI 활용을 보장하다가 기밀 접촉 직후에만 동적으로 '선제 감시(WATCH)' 모드를 켜는 Zero Trust 적응형 보안을 실현했습니다.
