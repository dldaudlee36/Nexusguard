...
if menu == "대시보드 종합 관제":
    ...
    <div style="margin-bottom: 20px;">
        <h1 style="margin:0; font-size: 26px; font-weight: 700; color:#ffffff;">Shadow AI Dashboard</h1>
    ...
    with kpi1:
        ...
        <div class="kpi-card">
            <div class="kpi-title">
                {tooltip("📊", "전체 인시던트 집계", "수집된 1,248건의 원천 이벤트 중 다차원 상관분석을 통해 최종 압축 식별된 침해사고 건수입니다.")} 전체 인시던트
            </div>
            <div class="kpi-value">{len(incidents)}</div>
            <div class="kpi-sub">총 1,248건 이벤트 상관분석 압축</div>
        </div>
        """, unsafe_allow_html=True)
    with kpi2:
        crit_count = sum(1 for inc in incidents if inc.severity in [Severity.CRITICAL, Severity.HIGH])
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-title critical-text">
                {tooltip("🚨", "CRITICAL / HIGH 긴급 경보", "즉각적인 격리 또는 차단이 요구되는 활성 공격 및 기밀 유출 사건입니다.")} CRITICAL / HIGH
            </div>
            <div class="kpi-value critical-text">{crit_count}</div>
            <div class="kpi-sub">긴급 대응 필요 침해 킬체인</div>
        </div>
        """, unsafe_allow_html=True)
    with kpi3:
        med_count = sum(1 for inc in incidents if inc.severity == Severity.MEDIUM)
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-title medium-text">
                {tooltip("⚠️", "MEDIUM 주의 단계", "비인가 포트 스캔, 마이닝 풀 질의 등 정찰 행위 및 정책 위반 사항입니다.")} MEDIUM (주의)
            </div>
            <div class="kpi-value medium-text">{med_count}</div>
    ...
    with kpi4:
        ...
        <div class="kpi-card">
            <div class="kpi-title low-text">
                {tooltip("🤖", "섀도우 AI·IT 자산", "사내 DNS 질의 분석으로 포착된 미승인 생성형 AI 및 SaaS 도메인 수입니다.")} 섀도우 AI / SaaS 자산
            </div>
            <div class="kpi-value low-text">{len(shadow_assets)}</div>
            <div class="kpi-sub">DNS 기반 사내 미승인 서비스 식별</div>
        </div>
        """, unsafe_allow_html=True)

    # =========================================================================
    # 🌟 1. 최근 주요 인시던트 목록 (전체 너비 100% 확장 및 시인성 극대화)
    # =========================================================================
    header_title = f"""
    <div class="box-title">
        {tooltip("🚨", "최근 주요 인시던트", "상관분석 엔진이 이기종 로그를 교차 분석하여 공격 킬체인으로 묶은 실시간 침해사고 목록입니다.")} 최근 주요 인시던트 목록 (총 {len(incidents)}건)
    </div>
    """
    st.markdown(header_title, unsafe_allow_html=True)

    # 🌟 시인성이 개선된 드롭다운 옵션 포맷터 함수
    ...
    # 인시던트 요약 테이블 렌더링 (전체 100% 폭 활용, 잘림 없이 명확한 정보 제공)
    table_rows = []
    for inc in incidents:
        ...
        table_rows.append({
            "인시던트 ID": inc.incident_id,
            "위험도": f"{sev_emoji} {inc.severity.value}",
            "카테고리": inc.category.value,
            "공격 명칭": inc.title,
            "행위자 / 발원지": inc.actor,
            "요약": inc.summary,
            "상태": "진행 중" if inc.status == IncidentStatus.ACTIVE else ("격리" if inc.status == IncidentStatus.CONTAINED else "조치 완료")
        })

    st.markdown("""
    <div style="margin-bottom:8px;">
        <span style="font-size:13px; color:#94a3b8;">
            💡 <b>인시던트 목록에서 행을 직접 클릭</b>하거나 <b>빠른 선택 버튼 / 드롭다운</b>을 선택하면 하단의 <b>네트워크 토폴로지 맵</b> 및 <b>상세 공격 분석</b>이 즉시 해당 인시던트로 전환됩니다.
        </span>
    </div>
    """, unsafe_allow_html=True)
    ...
    df_event = st.dataframe(
        ...
        key="incident_table_selector"
    )

    # 표 행 클릭 시 세션 상태 동기화 (rerun 호출 없이 자동 반영)
    if "incident_table_selector" in st.session_state:
        sel = st.session_state.incident_table_selector.get("selection", {})
        rows = sel.get("rows", [])
        if rows:
            clicked_idx = rows[0]
            if 0 <= clicked_idx < len(incidents):
                st.session_state.selected_incident_id = incidents[clicked_idx].incident_id

    # 🌟 10대 인시던트 원클릭 빠른 선택 버튼 바 (즉시 토폴로지 맵 & 공격 분석 전환)
    st.markdown("<div style='font-size:13px; font-weight:700; color:#94a3b8; margin: 10px 0 6px 0;'>⚡ 빠른 인시던트 선택 (클릭 즉시 토폴로지 및 상세 분석 전환):</div>", unsafe_allow_html=True)
    quick_cols = st.columns(len(incidents))
    for i, inc in enumerate(incidents):
        with quick_cols[i]:
            is_active = (inc.incident_id == st.session_state.selected_incident_id)
            btn_label = f"▶ {inc.incident_id}" if is_active else inc.incident_id
            st.button(
                btn_label,
                key=f"quick_btn_{inc.incident_id}",
                on_click=select_incident_cb,
                args=(inc.incident_id,),
                type="primary" if is_active else "secondary",
                use_container_width=True,
                help=f"[{inc.severity.value}] {inc.title}"
            )

    # 드롭다운 선택기 & 심층 킬체인 전용 화면 바로가기 바
    def on_dropdown_select():
        st.session_state.selected_incident_id = st.session_state.incident_dropdown_selector

    if st.session_state.selected_incident_id not in incident_ids:
        st.session_state.selected_incident_id = incident_ids[0]
    curr_idx = incident_ids.index(st.session_state.selected_incident_id)
    st.session_state.incident_dropdown_selector = st.session_state.selected_incident_id

    col_sel, col_action = st.columns([7, 3])
    with col_sel:
        selected_id = st.selectbox(
            "💡 상세 분석할 인시던트를 선택하세요 (드롭다운)",
            options=incident_ids,
            format_func=format_inc_option,
            key="incident_dropdown_selector",
            on_change=on_dropdown_select,
            help="인시던트를 선택하면 하단 네트워크 토폴로지 맵과 상세 공격 타임라인 및 판단 근거가 실시간으로 동적 갱신됩니다."
        )
    ...
    with col_action:
        ...
        st.button(
            f"🎯 {selected_inc.incident_id} 킬체인 심층 분석 전용 화면으로 이동",
            on_click=navigate_to,
            args=("침해사고 킬체인 분석", selected_inc.incident_id),
            use_container_width=True,
            help="해당 인시던트의 침해사고 킬체인 분석 전용 화면으로 즉시 전환합니다."
        )

    st.markdown("<hr style='border:none; border-top:1px solid #1c2e47; margin:22px 0 16px 0;'>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-size:12px; color:#9fb0c8; text-align:center; margin-top:6px;'>현재 선택: <b>{selected_inc.incident_id}</b> ({selected_inc.category.value}) 침해 경로 시각화</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    # 10대 인시던트 퀵 버튼 (킬체인 분석 화면에서도 자유롭게 전환)
    st.markdown("<div style='font-size:13px; font-weight:700; color:#94a3b8; margin: 6px 0 6px 0;'>⚡ 인시던트 즉시 전환:</div>", unsafe_allow_html=True)
    quick_k_cols = st.columns(len(incidents))
    for i, inc in enumerate(incidents):
        with quick_k_cols[i]:
            is_active = (inc.incident_id == st.session_state.selected_incident_id)
            btn_label = f"▶ {inc.incident_id}" if is_active else inc.incident_id
            st.button(
                btn_label,
                key=f"k_quick_btn_{inc.incident_id}",
                on_click=select_incident_cb,
                args=(inc.incident_id,),
                type="primary" if is_active else "secondary",
                use_container_width=True,
